...
namespace CryDialog.Editor
{
    public class NodeEditor<T> : Singleton<T> where T : class, new()
    {
        ...
        protected DialogEditorWindow _window;
        ...
        public void OnGUI(DialogEditorWindow window, NodeModifier[] nodes)
        {
        ...
        {
            _window.Zoom = GUI.HorizontalSlider(new Rect(_contentRect.x + 3, _contentRect.y, 200, 20), _window.Zoom, DialogEditorWindow._zoomMin, DialogEditorWindow._zoomMax);
        ...
        protected virtual void DrawLeftArribute(object o)
        {
            ...
            System.Reflection.FieldInfo[] fileds = _window._dialogObject._debugMode ? type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance) : type.GetFields();

            for (int i = 0; i < fileds.Length; i++)
            {
                System.Reflection.FieldInfo filed = fileds[i];
                if (filed.Name.StartsWith("_") && !_window._dialogObject._debugMode) continue;
        ...
        private bool DrawKeyRefernceControl(FieldInfo filed, Rect rect, object o)
        {
            ...
            var = VarRefernce.GetValue(dialog, key);
        ...
        protected void DuplicateNode(NodeModifier targetNode)
        {
            ...
            UniqueIDCalculator d = node.GetContent() as UniqueIDCalculator;
